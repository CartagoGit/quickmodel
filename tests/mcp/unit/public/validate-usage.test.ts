...
describe('QValidateUsageTool', () => {
    ...
    it('should detect missing QModel inheritance', async () => {
        ...
        expect(result.issues).toContain('Class should extend QModel<Interface>');
    });
    ...
    it('should detect missing declare keyword', async () => {
        ...
        expect(result.valid).toBe(false);
        expect(result.issues).toContain('Properties in QModel classes should be defined with "declare"');
    });
    ...
    it('should detect missing @Quick decorator', async () => {
        ...
        const result = await tool.execute({ code });
        expect(result.valid).toBe(false);
        expect(result.issues).toContain('Class should be decorated with @Quick (or properties with @QType)');
    });
});
