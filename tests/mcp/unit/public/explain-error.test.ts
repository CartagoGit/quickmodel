...
describe('QExplainErrorTool', () => {
    ...
    it('should explain INVALID_TYPE error', async () => {
        ...
        expect(result.explanation).toContain("Field 'user.age' expected number but got string.");
    ...
    it('should explain REQUIRED error', async () => {
        ...
        const result = await tool.execute({ error: JSON.stringify(error) });
        expect(result.explanation).toContain("Field 'user.name' is required but was missing.");
    });
    ...
    it('should handle array of errors', async () => {
        ...
        const errors = [
            ...
            { code: 'INVALID_TYPE', path: 'b', expected: 'Date', received: 'null' }
        ];
        const result = await tool.execute({ error: JSON.stringify(errors) });
        expect(result.explanation).toContain("Found 2 issues:");
        expect(result.explanation).toContain("- Field 'a' is required but was missing.");
        expect(result.explanation).toContain("- Field 'b' expected Date but got null.");
    });
    ...
    it('should handle object with errors array property', async () => {
        ...
        expect(result.explanation).toContain("Field 'x' is required but was missing.");
    });
});
