...
describe('QRule timeout — integration (real delays)', () => {
	...
	test(
		...
		async () => {
			...
			const result = await user.$qCheckRulesAsync(tightOptions);
	...
	test(
		...
		async () => {
			...
			const result = await user.$qCheckRulesAsync(); // no timeout
	...
	test(
		...
		async () => {
			...
			const valid = await user.$qIsValidAsync(tightOptions);
	...
	test(
		...
		async () => {
			...
			const report = await user.$qValidationReportAsync(tightOptions);
	...
	test(
		...
		async () => {
			...
			const result2 = await model.$qCheckRulesAsync({
				timeoutMs: 30,
				timeoutMessage: () => `${serviceName} did not respond in time`,
			});
