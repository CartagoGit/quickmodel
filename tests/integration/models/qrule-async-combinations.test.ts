...
describe('no timeoutMs — all predicates awaited to completion', () => {
	...
	test(
		...
		async () => {
			...
			expect((await model.$qCheckRulesAsync()).valid).toBe(true);
		},
		{ timeout: 500 }
	);
});
...
describe('varying timeoutMs across calls on the same model', () => {
	...
	test(
		...
		async () => {
			const result = await model.$qCheckRulesAsync({ timeoutMs: 50 });
			expect(
	...
	test(
		...
		async () => {
			const result = await model.$qCheckRulesAsync({ timeoutMs: 120 });
			expect(result.valid).toBe(true);
		},
		{ timeout: 500 }
	);
...
describe('sync and async rules coexist — sync unaffected by timeoutMs', () => {
	...
	test(
		...
		async () => {
			...
			const result = await model.$qCheckRulesAsync({ timeoutMs: 400 });
...
describe('full combination: heterogeneous delays, timeouts, outcomes', () => {
	...
	beforeAll(async () => {
		...
		result = await model.$qCheckRulesAsync(budget);
	});
	...
	test(
		...
		async () => {
			...
			await model.$qCheckRulesAsync(budget);
...
describe('isValidAsync and validationReportAsync with all option variants', () => {
	...
	test(
		...
		async () => {
			...
			expect(await model.$qIsValidAsync({ timeoutMs: 30 })).toBe(false);
		},
		{ timeout: 500 }
	);
	...
	test(
		...
		async () => {
			...
			expect(await model.$qIsValidAsync({ timeoutMs: 300 })).toBe(true);
		},
		{ timeout: 500 }
	);
	...
	test(
		...
		async () => {
			...
			expect(await model.$qIsValidAsync()).toBe(true);
		},
		{ timeout: 500 }
	);
	...
	test(
		...
		async () => {
			...
			expect(await model.$qIsValidAsync({ timeoutMs: 300 })).toBe(false);
		},
		{ timeout: 500 }
	);
