...
describe('Constructor Aliases', () => {
	...
	describe('Usando constructores nativos (@QType(RegExp), @QType(Error), etc.)', () => {
		...
		test('Should perform round-trip correctly', () => {
			...
			expect(Array.from(restored.bytes1)).toEqual(
				Array.from(original.bytes1)
			);
			expect(Array.from(restored.bytes2)).toEqual(
				Array.from(original.bytes2)
			);
		});
	});
	...
	describe('Both approaches should produce the same result', () => {
		test('Serialization should be identical', () => {
			...
			const serialized1 = model1.$qSerialize();
		...
		test('Deserialization should be identical', () => {
			...
			expect(Array.from(model1.bytes1)).toEqual(
				Array.from(model2.bytes1)
			);
			expect(Array.from(model1.bytes2)).toEqual(
				Array.from(model2.bytes2)
			);
		});
	});
});
