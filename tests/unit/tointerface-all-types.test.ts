...
interface AllTypesInterface {
	...
	objectDeeplyNested: { level1: { level2: { level3: { value: number } } } };
...
class AllTypesModel extends QModel<AllTypesInterface> {
	...
	declare objectDeeplyNested: { level1: { level2: { level3: { value: number } } } };
...
describe('toInterface() - All Types Preservation', () => {
	...
	test('should preserve all string types', () => {
		const data = {
			...
			objectWithNull: { x: null, y: undefined },
			objectNoProto: Object.create(null),
			objectDeeplyNested: { level1: { level2: { level3: { value: 1 } } } },
			dateISO: '2024-01-01T00:00:00.000Z',
	...
	test('should preserve bigint in different formats', () => {
		const data = {
			...
			bigintObject: { __type: 'bigint', value: '123456789012345678901234567890' },
			symUnique: Symbol('test'),
			symGlobal: Symbol.for('global'),
			symWellKnown: Symbol.iterator,
			nullValue: null,
		...
		expect(result.bigintObject).toEqual({ __type: 'bigint', value: '123456789012345678901234567890' });
	...
	test('should preserve arrays with nested structures', () => {
		const data = {
			...
			arrayWithWrappers: [new Number(42), new String('test'), new Boolean(true)],
			plainObject: { a: 1, b: '', nested: { c: true } },
			objectWithNull: { x: null, y: undefined },
			objectNoProto: Object.create(null),
			objectDeeplyNested: { level1: { level2: { level3: { value: 1 } } } },
	...
	test('should preserve Map serialized as array of pairs', () => {
		const data = {
			...
			regexpPattern: 'test',
			regexpObject: { source: 'test', flags: '' },
			mapAsArray: [['name', 'John'], ['age', 30], ['active', true]],
			mapEmpty: [],
			setAsArray: [],
			setEmpty: []
		};
	...
	test('should preserve deeply nested objects', () => {
		...
		expect(Number.isNaN(result.objectDeeplyNested.level1.level2.level3.nested.x)).toBe(true);
	});
