    ...
    describe('MapTransformer', () => {
        ...
        describe('deserialize', () => {
             test('should return Map as-is', () => {
                 const map = new Map();
                 expect(transformer.deserialize(map, propertyKey, className)).toBe(map);
             });
             ...
             test('should deserialize __type wrapper', () => {
                 const wrapper = { __type: 'Map' as const, entries: [['a', 1] as [string, unknown]] };
                 const result = transformer.deserialize(wrapper, propertyKey, className);
                 expect(result.get('a')).toBe(1);
             });

             test('should deserialize array of entries', () => {
                 const entries = [['a', 1], ['b', 2]] as [string, unknown][];
                 const result = transformer.deserialize(entries, propertyKey, className);
                 expect(result.get('b')).toBe(2);
             });
        ...
        describe('validate', () => {
            test('should validate Map', () => {
                expect(transformer.validate(new Map(), { propertyKey, target: {} }).isValid).toBe(true);
            });
            test('should validate object', () => {
                expect(transformer.validate({}, { propertyKey, target: {} }).isValid).toBe(true);
            });
            test('should validate array', () => {
                expect(transformer.validate([], { propertyKey, target: {} }).isValid).toBe(true);
            });
            test('should fail for number', () => {
                expect(transformer.validate(123, { propertyKey, target: {} }).isValid).toBe(false);
            });
        });
    });
    ...
    describe('SetTransformer', () => {
        ...
        describe('deserialize', () => {
             test('should return Set as-is', () => {
                 const set = new Set();
                 expect(transformer.deserialize(set, propertyKey, className)).toBe(set);
             });
        ...
        describe('validate', () => {
            test('should validate Set', () => {
                expect(transformer.validate(new Set(), { propertyKey, target: {} }).isValid).toBe(true);
            });
            test('should validate array', () => {
                expect(transformer.validate([], { propertyKey, target: {} }).isValid).toBe(true);
            });
            test('should validate __type wrapper', () => {
                expect(transformer.validate({ __type: 'Set', values: [] }, { propertyKey, target: {} }).isValid).toBe(true);
            });
            test('should fail for plain object', () => {
                 // Plain object that is not a wrapper
                expect(transformer.validate({ foo: 'bar' }, { propertyKey, target: {} }).isValid).toBe(false);
            });
             test('should fail for number', () => {
                expect(transformer.validate(123, { propertyKey, target: {} }).isValid).toBe(false);
            });
