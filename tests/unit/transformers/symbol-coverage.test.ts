...
describe('Transformer Coverage: SymbolTransformer', () => {
    ...
    describe('deserialize', () => {
        ...
        test('should throw error for object wrapper with invalid description', () => {
             expect(() => {
                // @ts-expect-error - Testing invalid inputs
                transformer.deserialize({ __type: 'symbol', description: 123 }, propertyKey, className);
    ...
    describe('serialize', () => {
        test('should serialize global symbol to object with key', () => {
            ...
            expect(result).toEqual({ __type: 'symbol', description: 'global-key' });
        });
    ...
    describe('validate', () => {
        test('should validate symbol', () => {
            const result = transformer.validate(Symbol(), { propertyKey, target: {} });
            expect(result.isValid).toBe(true);
        });

        test('should validate string', () => {
            const result = transformer.validate('key', { propertyKey, target: {} });
            expect(result.isValid).toBe(true);
        });

        test('should validate object wrapper', () => {
             const result = transformer.validate({ __type: 'symbol', description: 'ok' }, { propertyKey, target: {} });
             expect(result.isValid).toBe(true);
        });

        test('should fail validation for invalid type', () => {
            const result = transformer.validate(123, { propertyKey, target: {} });
