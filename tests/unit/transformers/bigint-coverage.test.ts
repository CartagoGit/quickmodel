...
describe('Transformer Coverage: BigIntTransformer', () => {
    ...
    describe('validate', () => {
        test('should validate bigint instance', () => {
            const result = transformer.validate(123n, { propertyKey, target: {} });
            expect(result.isValid).toBe(true);
        });

        test('should validate numeric string', () => {
            const result = transformer.validate('123456', { propertyKey, target: {} });
            expect(result.isValid).toBe(true);
        });
        ...
        test('should fail validation for non-numeric string', () => {
            const result = transformer.validate('abc', { propertyKey, target: {} });
            expect(result.isValid).toBe(false);
        });

        test('should fail validation for boolean', () => {
            const result = transformer.validate(true, { propertyKey, target: {} });
            expect(result.isValid).toBe(false);
        });
        
        test('should fail validation for object', () => {
             const result = transformer.validate({}, { propertyKey, target: {} });
             expect(result.isValid).toBe(false);
        });
    });
});
