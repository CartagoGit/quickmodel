...
describe('Transformer Coverage: ErrorTransformer', () => {
    ...
    describe('deserialize', () => {
        ...
        test('should throw error if message is not string', () => {
             expect(() => {
                // @ts-expect-error - Testing invalid inputs
                transformer.deserialize({ message: 123 }, propertyKey, className);
    ...
    describe('validate', () => {
        test('should validate Error instance', () => {
            const result = transformer.validate(new Error(), { propertyKey, target: {} });
            expect(result.isValid).toBe(true);
        });

        test('should validate string', () => {
            const result = transformer.validate('my error', { propertyKey, target: {} });
            expect(result.isValid).toBe(true);
        });

        test('should validate valid object', () => {
             const result = transformer.validate({ message: 'ok' }, { propertyKey, target: {} });
             expect(result.isValid).toBe(true);
        });
        ...
        test('should fail validation for object without message', () => {
             const result = transformer.validate({ other: 'prop' }, { propertyKey, target: {} });
