...
describe("getSchema('drizzle') — tipos primitivos", () => {
	test('produce un pgTable con columnas correctas', () => {
		const result = UserSchema.getSchema('drizzle') as string;
		expect(result).toContain("pgTable('userschema'");
	...
	test('Date → timestamp en drizzle', () => {
		const result = ProductSchema.getSchema('drizzle') as string;
	...
	test('Array → jsonb en drizzle', () => {
		const result = StatsSchema.getSchema('drizzle') as string;
...
describe("getSchema('typebox') — tipos primitivos", () => {
	test('produce un Type.Object con los campos correctos', () => {
		const result = UserSchema.getSchema('typebox') as string;
	...
	test('Date → Type.String({ format: "date-time" }) en typebox', () => {
		const result = ProductSchema.getSchema('typebox') as string;
		expect(result).toContain('createdAt:');
		expect(result).toContain("Type.String({ format: 'date-time' })");
	});

	test('Array → Type.Array(Type.Unknown()) en typebox', () => {
		const result = StatsSchema.getSchema('typebox') as string;
		expect(result).toContain('tags:');
		expect(result).toContain('Type.Array(');
	});

	test('el resultado es un string que empieza con const', () => {
		const result = UserSchema.getSchema('typebox') as string;
		expect(typeof result).toBe('string');
		expect(result.startsWith('const')).toBe(true);
	});
