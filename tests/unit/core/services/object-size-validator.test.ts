...
describe('ObjectSizeValidator', () => {
	...
	describe('validateNestedObjectSize', () => {
		...
		it('should pass for normal object', () => {
			const obj = { a: 1 };
			expect(() =>
				validator.validateNestedObjectSize('prop', obj, CLASS_NAME)
			).not.toThrow();
		});

		it('should ignore primitives', () => {
			expect(() =>
				validator.validateNestedObjectSize('prop', 123, CLASS_NAME)
			).not.toThrow();
			expect(() =>
				validator.validateNestedObjectSize('prop', 'string', CLASS_NAME)
			).not.toThrow();
			expect(() =>
				validator.validateNestedObjectSize('prop', null, CLASS_NAME)
			).not.toThrow();
			expect(() =>
				validator.validateNestedObjectSize(
					'prop',
					undefined,
					CLASS_NAME
				)
		...
		it('should ignore special types (Date, RegExp, Map, Set)', () => {
			...
			expect(() =>
				validator.validateNestedObjectSize('prop', /abc/, CLASS_NAME)
			).not.toThrow();
			expect(() =>
				validator.validateNestedObjectSize(
					'prop',
