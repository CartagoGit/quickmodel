...
describe('Nested Model Validation', () => {
    ...
    class ValidatingTransformer {
        serialize(v: any) { return v; }
        deserialize(v: any) { return v; }
        validate(v: any) {
             if (v === 'INVALID_VALUE') return { isValid: false, error: 'Value is invalid' };
             return { isValid: true };
        }
    }
    
    QTransformerRegistry.register('ValidationType', new ValidatingTransformer());
