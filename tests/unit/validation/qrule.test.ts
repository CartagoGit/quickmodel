...
class UserModel extends QModel<IUser> {
	@QRule((v: unknown) => typeof v === 'string' && (v as string).length >= 3, 'Name must be at least 3 chars')
	declare name: string;

	@QRule((v: unknown) => typeof v === 'number' && (v as number) >= 0, 'Age cannot be negative')
	@QRule((v: unknown) => typeof v === 'number' && (v as number) <= 120, 'Age must be realistic')
	declare age: number;

	@QRule((v: unknown) => typeof v === 'string' && (v as string).includes('@'), 'Must be a valid email')
...
describe('@QRule + checkRules()', () => {
	...
	test('should return valid=false and collect errors when rules fail', () => {
		const user = new UserModel({ name: 'Jo', age: -1, email: 'notanemail' });
		const result = user.checkRules();
	...
	test('should report the correct field name', () => {
		const user = new UserModel({ name: 'Jo', age: 30, email: 'valid@email.com' });
		const result = user.checkRules();
	...
	test('should collect ALL failures for a field with multiple rules', () => {
		...
		const user = new UserModel({ name: 'Alice', age: 999, email: 'alice@example.com' });
		const result = user.checkRules();
		expect(result.valid).toBe(false);
...
describe('@QRule — lazy message (() => string)', () => {
	...
	class LocalizedModel extends QModel<{ name: string }> {
		@QRule((v: unknown) => typeof v === 'string' && (v as string).length >= 3, () => t('val.min'))
		declare name: string;
	}
...
describe('@QRule — edge cases', () => {
	...
	test('predicate receiving null/undefined returns the rule message', () => {
		...
		class NullModel extends QModel<{ val: string }> {
			@QRule((v: unknown) => v !== null && v !== undefined && (v as string).length > 0, 'Required')
			declare val: string;
		}
