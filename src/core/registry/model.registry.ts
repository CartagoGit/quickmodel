...
export class QModelRegistry implements IQModelRegistry {
  ...
  register(modelClass: ModelConstructor, properties: string[]): void {
    ...
    if (existing && existing !== modelClass) {
      // CRITICAL: Signature collision means ambiguous auto-inference
      // This would cause incorrect deserialization in production
      throw new Error(
        `QModelRegistry: Signature collision detected!\n` +
        `Signature "${signature}" is already registered for "${existing.name}".\n` +
        `Cannot register "${modelClass.name}" with the same properties.\n\n` +
        `⚠️  Auto-inference requires unique property combinations.\n` +
        `Solutions:\n` +
        `  1. Add unique properties to differentiate models\n` +
        `  2. Use @QType(${modelClass.name}) explicitly for arrays\n` +
        `  3. Call qModelRegistry.clear() in test setup (beforeEach)\n\n` +
