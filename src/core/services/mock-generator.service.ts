...
export class QMockGenerator {
	...
	/**
	 * Collects all property names registered via `@QType()` or `@Quick()` on the instance's prototype chain.
	 * @internal
	 * @param instance - A model instance (used to traverse the prototype chain for metadata)
	 * @returns Deduplicated array of decorated property names
	 */
	...
	/**
	 * Generates a single mock value for the given field metadata.
	 * Dispatches to `generateByFieldType()` when `fieldType` is set, otherwise uses `designType`.
	 *
	 * @param options - Field metadata including `type`, `fieldType`, `designType`, and `arrayElementClass`
	 * @returns A generated mock value appropriate for the field
	 */
	...
	/**
	 * Generates a mock value based on a QuickModel `fieldType` metadata token (e.g. `Date`, `'bigint'`).
	 *
	 * @param type - The mock quality level (`'empty'`, `'sample'`, or `'random'`)
	 * @param fieldType - The transformer key: a symbol, string, or constructor function
	 * @returns A generated mock value matching the transformer type
	 */
	...
	/**
	 * Generates a mock value based on the TypeScript `design:type` constructor inferred via `reflect-metadata`.
	 *
	 * @param type - The mock quality level (`'empty'`, `'sample'`, or `'random'`)
	 * @param designType - The constructor function from `design:type` metadata (e.g. `String`, `Number`, `Date`)
	 * @returns A generated mock value matching the design type
	 */
	...
	/**
	 * Routes mock generation to the appropriate quality-level helper based on `type`.
	 *
	 * @param type - Mock quality level: `'empty'` → empty/zero values, `'sample'`/`'minimal'` → readable samples, `'random'` → faker-generated values
	 * @param jsType - Normalized lowercase type name (e.g. `'string'`, `'date'`, `'map'`)
	...
	/**
	 * Returns an 'empty' mock value for the given JS type (zero, empty string, empty collection, etc.).
	 *
	 * @param jsType - Normalized lowercase type name (e.g. `'string'`, `'date'`, `'set'`)
	 * @returns The empty/zero value for the type, or `null` if the type is unknown
	 */
	...
	/**
	 * Returns a deterministic, human-readable sample value for the given JS type.
	 *
	 * @param jsType - Normalized lowercase type name (e.g. `'string'`, `'date'`, `'set'`)
	 * @returns A predictable sample value (e.g. `'sample'`, `42`, a fixed Date ISO string), or `null` if unknown
	 */
	...
	/**
	 * Returns a randomised mock value using `@faker-js/faker` for the given JS type.
	 *
	 * @param jsType - Normalized lowercase type name (e.g. `'string'`, `'number'`, `'date'`)
	 * @returns A randomised value generated by faker, or `null` if the type is unknown
	 */
