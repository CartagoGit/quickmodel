...
> implements IQDeserializer<TInterface, TModel> {
	 ...
	 */
	private populateInstance<T extends Record<string, unknown>>(
		...
		for (const [key, value] of Object.entries(data)) {
			...
			if (arrayElementClass) {
				...
				if (
					...
					if (isPrimitiveOrTransformable && !discriminators?.[key]) {
						...
						instance[key] = this.transformNestedArray(
							value,
							arrayElementClass,
							context
						);
				...
				if (!isArrayType && arrayElementClass === Set) {
					...
					if (setTransformer) {
						instance[key] = setTransformer.deserialize(
							value,
							context.propertyKey,
							context.className
						);
				...
				if (!isArrayType && arrayElementClass === Map) {
					...
					if (mapTransformer) {
						instance[key] = mapTransformer.deserialize(
							value,
							context.propertyKey,
							context.className
						);
			...
			if (
				...
				instance[key] = validItems.map((item: unknown) => {
					if (typeof item !== 'object' || item === null) {
						throw new Error(
							`${context.className}.${key}[]: Expected object, got ${typeof item}`
						);
					}
			...
			if (
				...
				instance[key] = this.deserializeArrayWithInference(
					value,
					context
				);
				continue;
			...
			if (
				...
				if (designType === Set || designType === Map) {
					instance[key] = this.transformByDesignType(
						value,
						designType,
						context
					);
				...
				const isTransformableType =
					transformableTypes.includes(designType);
		...
		if (arrayElementClass) {
			if (Array.isArray(value)) {
				...
				if (transformableTypes.includes(arrayElementClass as unknown)) {
					current[lastKey] = this.transformNestedArray(
						value,
						arrayElementClass,
						context
					);
					...
					// Model Array
					current[lastKey] = this.transformNestedModelArray(
						value,
						[arrayElementClass],
						undefined
					);
	 ...
	 */
	private deserializeArrayWithInference(
		value: unknown[],
	...
	private transformByDesignType(
		...
		if (designType === BigInt) {
			...
			throw new Error(
				`${context.className}.${context.propertyKey}: Invalid BigInt value`
			);
		}
		...
		if (designType === Symbol) {
			...
			throw new Error(
				`${context.className}.${context.propertyKey}: Invalid Symbol value`
			);
		}
		...
		if (designType === Map) {
			...
			throw new Error(
				`${context.className}.${context.propertyKey}: Invalid Map value`
			);
		}
		...
		if (designType === Set) {
			...
			throw new Error(
				`${context.className}.${context.propertyKey}: Invalid Set value`
			);
		}
		...
		if (designType === RegExp) {
			...
			throw new Error(
				`${context.className}.${context.propertyKey}: RegExp requires transformer`
			);
		}
		...
		if (designType === Error) {
			...
			throw new Error(
				`${context.className}.${context.propertyKey}: Error requires transformer`
			);
		}
		...
		if (designType === Uint8Array) {
			...
			throw new Error(
				`${context.className}.${context.propertyKey}: Uint8Array requires transformer`
			);
		}
		...
		if (designType === Uint16Array) {
			...
			throw new Error(
				`${context.className}.${context.propertyKey}: Uint16Array requires transformer`
			);
		}
		...
		if (designType === Uint32Array) {
			...
			throw new Error(
				`${context.className}.${context.propertyKey}: Uint32Array requires transformer`
			);
		}
		...
		if (designType === Int8Array) {
			...
			throw new Error(
				`${context.className}.${context.propertyKey}: Int8Array requires transformer`
			);
		}
		...
		if (designType === Int16Array) {
			...
			throw new Error(
				`${context.className}.${context.propertyKey}: Int16Array requires transformer`
			);
		}
		...
		if (designType === Int32Array) {
			...
			throw new Error(
				`${context.className}.${context.propertyKey}: Int32Array requires transformer`
			);
		}
		...
		if (designType === Float32Array) {
			...
			throw new Error(
				`${context.className}.${context.propertyKey}: Float32Array requires transformer`
			);
		}
		...
		if (designType === Float64Array) {
			...
			throw new Error(
				`${context.className}.${context.propertyKey}: Float64Array requires transformer`
			);
		}
		...
		if (designType === BigInt64Array) {
			...
			throw new Error(
				`${context.className}.${context.propertyKey}: BigInt64Array requires transformer`
			);
		}
		...
		if (designType === BigUint64Array) {
			...
			throw new Error(
				`${context.className}.${context.propertyKey}: BigUint64Array requires transformer`
			);
		}
		...
		if (designType === ArrayBuffer) {
			...
			throw new Error(
				`${context.className}.${context.propertyKey}: ArrayBuffer requires transformer`
			);
		}
		...
		if (designType === DataView) {
			...
			throw new Error(
				`${context.className}.${context.propertyKey}: DataView requires transformer`
			);
		}
		...
		const isNativeConstructor = (
			value: unknown
		): value is (typeof nativeConstructors)[number] => {
		...
		if (
			...
			return this.deserialize(
				...
				designType as ModelConstructor
			);
		}
		...
		if (isNativeConstructor(designType)) {
			...
			if (typeof designType !== 'function' || !('name' in designType)) {
				throw new Error(
					'Invalid native constructor: missing name property'
				);
			}
	...
	private resolveUnionType(
		...
		const getFirstType = (): Function => {
			...
			if (!firstType) {
				throw new Error(
					'resolveUnionType: possibleTypes[0] is undefined'
				);
			}
			return firstType;
		};
		...
		if (data !== null && data !== undefined) {
			const exactMatch = possibleTypes.find(
				(type) =>
					type !== String &&
					type !== Number &&
					type !== Boolean &&
		...
		if (typeof data === 'string' && possibleTypes.includes(String))
			return String;
		if (typeof data === 'number' && possibleTypes.includes(Number))
			return Number;
		if (typeof data === 'boolean' && possibleTypes.includes(Boolean))
			return Boolean;
		if (typeof data === 'bigint' && possibleTypes.includes(BigInt))
		...
		if (typeof discriminatorConfig === 'string') {
			...
			if (fieldValue !== undefined) {
				...
				const matched = possibleTypes.find((ctor) => {
					...
					if (typeof ctor !== 'function' || !('name' in ctor))
						return false;
					const ctorName = (
						ctor as { name: string }
					).name?.toLowerCase();
	...
	private transformNestedModelArray(
		...
		return nestedArray
			...
			.map((item) => {
				...
				return new (resolvedClass as ModelConstructor)(
					item as Record<string, unknown>
				);
			});
	}
