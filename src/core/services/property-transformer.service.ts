...
export class PropertyTransformer {
    ...
    public transformProperty(
        ...
        if (arrayElementClass) {
            ...
            if (
                ...
                const transformableTypes = [
                    Date, BigInt, Number, String, Boolean, RegExp, Symbol, Error, URL,
                    URLSearchParams, Int8Array, Uint8Array, Uint8ClampedArray,
            ...
            if ((arrayElementClass === Set || arrayElementClass === Map) && !arrayNestingDepth) {
                const IQTransformerKey = arrayElementClass === Set ? 'set' : 'map';
                const transformer = this.transformerLookup.getTransformer(IQTransformerKey);
                if (transformer) {
                    return transformer.deserialize(
                        value,
                        transformContext.propertyKey,
            ...
            if (isArrayType) {
                ...
                if (isPrimitiveOrTransformable && !hasDiscriminator) {
                    ...
                    const isTypedArrayElement = typedArrayConstructors.includes(arrayElementClass);
        ...
        const designType = Reflect.getMetadata('design:type', instance, targetKey);
        
        if (designType === Array && !arrayElementClass) {
             return value;
        }

        // Special Case: Set/Map/TypedArrays (Auto-wrap fallback or implicit handling)
        if (Array.isArray(value) && designType && designType !== Array && designType !== Object) {
            // ... Logic simplified, delegating to value transformer if needed or keeping as is
             // This part of PopulationService logic seemed to handle "Implicit" transformations if designType matched a transformer
             // But ValueTransformer.transformByDesignType does that too.
             // We can defer to transformByDesignType if we aren't sure.
             
             if (designType === Set || designType === Map) {
                  return this.valueTransformer.transformByDesignType(value, designType, transformContext);
             }
             
             // Check if designType is a TypedArray constructor
             const typedArrayConstructors = [
					Int8Array, Uint8Array, Uint8ClampedArray,
					Int16Array, Uint16Array, Int32Array, Uint32Array,
					Float32Array, Float64Array, BigInt64Array, BigUint64Array
			];
            if (typedArrayConstructors.includes(designType)) {
                 return this.valueTransformer.transformByDesignType(value, designType, transformContext);
            }
        }
