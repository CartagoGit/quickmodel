			...
			messages: [
				...
				this.assistant(
					`### FormData ↔ QModel — Integration Guide\n\n` +
						...
						`| \`dto.$qToFormData()\` | Build FormData from model fields |\n` +
						...
						`| Method | Purpose |\n` +
						`|---|---|\n` +
						`| \`dto.$qToReadableStream({ field, chunkSize?, onChunk? })\` | Emit model field as \`ReadableStream<Uint8Array>\` — never full file in memory |\n` +
						`| \`dto.$qToReadableStream({ multipart: true, onChunk? })\` | Emit all fields as a complete \`multipart/form-data\` stream |\n` +
