...
> Last updated: 2026-03-01 (sprint 8)
...
| Sprint | Fecha      | Vectores corregidos | Tests de seguridad | Checks estáticos | Pasada / Estado                                |
| ------ | ---------- | ------------------- | ------------------ | ---------------- | ---------------------------------------------- |
| 1      | 2026-02-10 | 8                   | ~150               | ~25              | CRIT-01, HIGH-01..04, MED-02..03, LOW-01..02   |
| 2      | 2026-02-12 | 6                   | ~175               | ~31              | MED-04..07, LOW-03..04, NEW-01..03             |
| 3      | 2026-02-14 | 9                   | ~210               | ~38              | MED-11, LOW-05..13, path guards adicionales    |
| 4      | 2026-02-17 | 8                   | 293                | 51               | MED-12, LOW-14..16, HIGH-05..06                |
| 5      | 2026-02-20 | 17                  | 313                | 60               | LOW-17..22, MED-13..14 + tests de regresión    |
| 6      | 2026-02-25 | 15                  | 343                | 75               | MED-15, MED-16a..d, LOW-23..31                 |
| 7      | 2026-02-28 | 7                   | 362                | 82               | MED-17..23, pasada 2 — 0 vectores nuevos       |
| 8      | 2026-03-01 | 4                   | **371**            | **86**           | MED-24, LOW-32..34, pasada 4 — ✅ **0 nuevos** |

### Estado actual

- **Tests de seguridad**: 371 pass, 0 fail (`bun test tests/security/` — 45 archivos)
- **Checks estáticos**: 86 (`run_security_audit` MCP tool)
- **Última pasada exhaustiva**: sprint 8 — ZERO NEW VULNERABILITIES FOUND
- **Gate de seguridad**: `check_security → passed: true`
...
| DoS — Input Size       | Unbounded string caps in `add_to_sidebar`                            | MED-17    | ✅ Fixed | add-to-sidebar.tool.ts                                                          |
| DoS — Memory           | Unbounded record keys/values + no key-count guard in `generate_mock` | MED-18/19 | ✅ Fixed | generate-mock.tool.ts                                                           |
| DoS — Memory           | No data key-count guard in `simulate_validation`                     | MED-20    | ✅ Fixed | simulate-validation.tool.ts                                                     |
| DoS — Memory           | No data key-count guard in `simulate_rules`                          | MED-21    | ✅ Fixed | simulate-rules.tool.ts                                                          |
| DoS — Memory           | No data key-count guard in `simulate_async_rules`                    | MED-22    | ✅ Fixed | simulate-async-rules.tool.ts                                                    |
| DoS — Input Size       | Unbounded `files[]` array in `pre_commit_check`                      | MED-23    | ✅ Fixed | pre-commit-check.tool.ts                                                        |
| DoS — Memory           | Unbounded `properties` record keys/values in `create_model`          | MED-24    | ✅ Fixed | create-model.tool.ts                                                            |
| DoS — Input Size       | Unbounded `targetDir` in `list_todos`                                | LOW-32    | ✅ Fixed | list-todos.tool.ts                                                              |
| DoS — Input Size       | Unbounded `baselineFile` in `check_api_compatibility`                | LOW-33    | ✅ Fixed | check-api-compat.tool.ts                                                        |
| DoS — Input Size       | Unbounded `tasks_path` in `manage_proposal`                          | LOW-34    | ✅ Fixed | manage-proposal.tool.ts                                                         |
...
### 34. MED-15 — Unbounded `timeoutMs` in `simulate_async_rules`

**Threat**: `timeoutMs` accepted any positive number. An attacker passing `timeoutMs: 9_999_999_999` would keep predicates suspended in-process indefinitely — worse than the process-timeout guard applies, because this timer operates inside the Node.js event loop.

**Defense**: `timeoutMs: z.number().max(60_000)` — maximum 60 seconds per predicate invocation.

**File**: `src/mcp/tools/public/simulate-async-rules.tool.ts`

---

### 35. MED-16a — Options Record Key-Count DoS in `check_integrity`

**Threat**: The `options` record accepted an unbounded number of keys. Each key triggers a full model construction (`@Quick` + decorator + `new DynamicField()`). With 100 000 keys the server performs 100 000 class constructions.

**Defense**: Runtime guard at the start of `execute()`: `if (Object.keys(args.options).length > 500) throw new Error('Security Error: too many options keys (max 500).')`.

**File**: `src/mcp/tools/public/check-integrity.tool.ts`

---

### 36. MED-16b — Options Record Key-Count DoS in `roundtrip`

**Threat**: Identical vector as MED-16a. The `options` record iterates all keys through `hydrateOptions()` and then into `@Quick()` decoration.

**Defense**: Same runtime guard: `Object.keys(args.options).length > 500 → throw`.

**File**: `src/mcp/tools/public/roundtrip.tool.ts`

---

### 37. MED-16c — Options Record Key-Count DoS in `simulate_transformation`

**Threat**: Identical vector. Options keys feed directly into `@Quick(hydratedOptions)`.

**Defense**: Same runtime guard at start of `execute()`.

**File**: `src/mcp/tools/public/simulate-transformation.tool.ts`

---

### 38. MED-16d — Options Record Key-Count DoS in `explain_transformation`

**Threat**: Identical vector, plus `allFields = new Set([...data keys, ...options keys])` meant options keys also flow into the trace loop.

**Defense**: Same runtime guard at start of `execute()`.

**File**: `src/mcp/tools/public/explain-transformation.tool.ts`

---

### 39. LOW-23 — Unbounded `file_path`, `symbol_name`, `jsdoc` in `patch_jsdoc`

**Threat**: No schema caps on the three main string inputs. A large `jsdoc` payload (e.g. 10 MB) would be written to disk after bypassing the path-traversal guard.

**Defense**: `file_path.max(500)`, `symbol_name.max(200)`, `jsdoc.max(100_000)`.

**File**: `src/mcp/tools/internal/patch-jsdoc.tool.ts`

---

### 40. LOW-24 — Unbounded String Fields in `manage_proposal`

**Threat**: `title`, `description`, `impact`, and `effort` had no `.max()` caps, allowing oversized content to be persisted to `TASKS.md`.

**Defense**: `title.max(200)`, `description.max(2000)`, `impact.max(500)`, `effort.max(200)`.

**File**: `src/mcp/tools/internal/manage-proposal.tool.ts`

---

### 41. LOW-25 — Unbounded `sourceFile` in `generate_test`

**Threat**: No schema cap on the path string. Arbitrarily long paths slowing down `resolve()` and regex operations before the safeCwd guard fires.

**Defense**: `sourceFile.max(500)`.

**File**: `src/mcp/tools/internal/generate-test.tool.ts`

---

### 42. LOW-26 — Unbounded `location` in `scaffold_feature`

**Threat**: `location` (target directory) had no Zod `.max()` cap despite the runtime path-traversal guard. Oversized strings enter resolve/join before being checked.

**Defense**: `location.max(500)`.

**File**: `src/mcp/tools/internal/scaffold-feature.tool.ts`

---

### 43. LOW-27 — Unbounded `pattern` in `run_tests`

**Threat**: Pattern string passed to `bun test <pattern>` had no schema-level length cap. A very long pattern could slow down bun's glob matching.

**Defense**: `pattern.max(500)`.

**File**: `src/mcp/tools/internal/run-tests.tool.ts`

---

### 44. LOW-28 — Unbounded `base_path` in `create_guide_page`

**Threat**: `base_path` (used to override project root for testing) had no `.max()` cap at schema level.

**Defense**: `base_path.max(500)`.

**File**: `src/mcp/tools/internal/create-guide-page.tool.ts`

---

### 45. LOW-29 — Unbounded `target_dir` Schema Cap in `deprecation_tracker`

**Threat**: The runtime safeCwd guard blocked path traversal, but the Zod schema accepted arbitrarily long strings that must be `resolve()`-d before the guard can fire.

**Defense**: `target_dir.max(500)` added at schema level as a first-line cap.

**File**: `src/mcp/tools/internal/deprecation-tracker.tool.ts`

---

### 46. LOW-30 — Unbounded `target_dir` Schema Cap in `validate_examples`

**Threat**: Identical to LOW-29. The safeCwd guard protects against traversal but not against oversized string inputs at the schema layer.

**Defense**: `target_dir.max(500)`.

**File**: `src/mcp/tools/internal/validate-examples.tool.ts`

---

### 47. LOW-31 — Unbounded `timeoutMessage` in `simulate_async_rules`

**Threat**: `timeoutMessage` had no `.max()` cap. With `rules.max(50)`, an attacker could supply a 50-rule payload each with a custom 1 MB timeout message, amplifying the server's response size by 50x.

**Defense**: `timeoutMessage.max(500)`.

**File**: `src/mcp/tools/public/simulate-async-rules.tool.ts`

---

### 48. MED-17 — Unbounded String Fields in `add_to_sidebar`

**Threat**: `slug`, `text_en`, `text_es`, `section_en`, `section_es`, and optional `config_path` had no `.max()` caps. Oversized values could bloat the sidebar config file written to disk.

**Defense**: `slug.max(100)`, `text_en/es.max(200)`, `section_en/es.max(200)`, `config_path.max(500)`.

**File**: `src/mcp/tools/internal/add-to-sidebar.tool.ts`

---

### 49. MED-18/19 — Unbounded Record Keys/Values and Key-Count DoS in `generate_mock`

**Threat**: `schema: z.record(z.string(), z.string())` allowed arbitrarily long keys/values and an unbounded number of fields. With 100 000 keys, `Quick(args.schema)(DynamicModel)` would register 100 000 decorators per call, causing OOM.

**Defense**: `z.record(z.string().max(100), z.string().max(100))` + runtime guard `if (Object.keys(args.schema).length > 200) throw`.

**File**: `src/mcp/tools/public/generate-mock.tool.ts`

---

### 50. MED-20 — No Data Key-Count Guard in `simulate_validation`

**Threat**: `data: z.record(z.string(), z.any())` with no key-count guard. With 500 000 keys the data is passed verbatim into rule evaluation and `new DynamicModel(data)`, causing OOM.

**Defense**: `if (Object.keys(args.data).length > 500) throw new Error('Security Error: too many data keys (max 500).')`.

**File**: `src/mcp/tools/public/simulate-validation.tool.ts`

---

### 51. MED-21 — No Data Key-Count Guard in `simulate_rules`

**Threat**: Same pattern as MED-20 but in the synchronous rules tool.

**Defense**: Same runtime guard at the top of `execute()`.

**File**: `src/mcp/tools/public/simulate-rules.tool.ts`

---

### 52. MED-22 — No Data Key-Count Guard in `simulate_async_rules`

**Threat**: Same pattern as MED-20 but in the async rules tool. Async execution makes the amplification effect worse since each async predicate resolves independently.

**Defense**: Same runtime guard immediately after destructuring `args`.

**File**: `src/mcp/tools/public/simulate-async-rules.tool.ts`

---

### 53. MED-23 — Unbounded `files[]` Array in `pre_commit_check`

**Threat**: The `files` array had per-item `.max(500)` and flag-injection refine, but no cap on the array length itself. An attacker could submit 100 000 file paths to cause unbounded ESLint/TSC spawning.

**Defense**: `.max(500)` on the array itself.

**File**: `src/mcp/tools/internal/pre-commit-check.tool.ts`

---

### 54. MED-24 — Unbounded `properties` Record Keys/Values in `create_model`

**Threat**: `properties: z.record(z.string(), z.string())` allowed arbitrarily long property names and type strings. A 50 MB value string would flow verbatim into the code-generation string concatenation, causing memory pressure or OOM.

**Defense**: `z.record(z.string().max(100), z.string().max(100))`.

**File**: `src/mcp/tools/public/create-model.tool.ts`

---

### 55. LOW-32 — Unbounded `targetDir` in `list_todos`

**Threat**: `targetDir: z.string().optional()` had no `.max()`. Even with the path-traversal guard, a multi-MB string is fully processed by `resolve()` and `startsWith()` before rejection.

**Defense**: `targetDir.max(500)`.

**File**: `src/mcp/tools/internal/list-todos.tool.ts`

---

### 56. LOW-33 — Unbounded `baselineFile` in `check_api_compatibility`

**Threat**: `baselineFile: z.string().optional()` had no `.max()`. Same pre-rejection work amplification for enormous path strings.

**Defense**: `baselineFile.max(500)`.

**File**: `src/mcp/tools/internal/check-api-compat.tool.ts`

---

### 57. LOW-34 — Unbounded `tasks_path` in `manage_proposal`

**Threat**: `tasks_path: z.string().optional()` had no `.max()`. All other fields in the same schema (`title`, `description`, `impact`, `effort`) were already capped.

**Defense**: `tasks_path.max(500)`.
...
| `mcp-input-caps.test.ts`         | LOW-05..31 + MED-11/13..16: parser/regex DoS caps, rules array, agent-coordinate, scaffold, list-todos, suggest-version-migration, lint-check, check-changelog, check-project-rules, create-model properties, explain-transformation, check-integrity, simulate-validation group, simulate-async-rules timeoutMs/timeoutMessage, options record key-count DoS (check-integrity/roundtrip/simulate-transformation/explain-transformation), patch-jsdoc caps, manage-proposal caps, generate-test/scaffold/run-tests/create-guide-page/deprecation-tracker/validate-examples path caps |
