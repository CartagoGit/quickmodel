...
export const scenarios: IBenchScenario[] = [
	{
		...
		notesEn:
			'10k validations — TypeBox compiles to raw JSON Schema checks (fastest), valibot is tree-shakeable, Zod is the most popular, yup is mature. QM adds auto-coercion on top of validation. Plain JS and class-transformer excluded: no schema validation.',
		notesEs:
			'10k validaciones — TypeBox compila a checks JSON Schema nativo (más rápido), valibot es tree-shakeable, Zod es el más popular, yup es maduro. QM añade coerción automática sobre la validación. Plain JS y class-transformer excluidos: sin validación de esquemas.',
	...
	},
	{
		...
		notesEn:
			'1k objects — auto vs manual coercion. QM: one decorator, zero extra code. Zod/valibot: manual transform per field. class-transformer: only Date via @Type, not BigInt/Map/Set. Plain JS, TypeBox and yup cannot do this at all.',
		notesEs:
			'1k objetos — coerción automática vs manual. QM: un decorador, cero código extra. Zod/valibot: transform manual por campo. class-transformer: solo Date via @Type, no BigInt/Map/Set. Plain JS, TypeBox y yup no pueden hacerlo.',
	...
	},
	{
		...
		notesEn:
			'1k round-trips. Plain JS (JSON.parse/stringify) is the fastest but loses Date→string, BigInt→error, Map/Set→{}. class-transformer preserves Date only via @Type. QuickModel preserves all types natively — the bar for Plain JS is clipped to keep the chart readable.',
		notesEs:
			'1k roundtrips. Plain JS (JSON.parse/stringify) es el más rápido pero pierde Date→string, BigInt→error, Map/Set→{}. class-transformer preserva solo Date via @Type. QuickModel preserva todos los tipos nativamente — la barra de Plain JS está recortada para que el gráfico sea legible.',
		appTypes: ['all', 'api', 'ddd', 'data'],
		values: {
			'Plain JS': 480_000,
	...
	},
	{
		...
		notesEn:
			'10 cycles of 1k objects — throughput validation. class-transformer excluded (not a validator — needs class-validator separately). Plain JS excluded (no validation).',
		notesEs:
			'10 ciclos de 1k objetos — rendimiento de validación en masa. class-transformer excluido (no es validador — necesita class-validator por separado). Plain JS excluido (sin validación).',
	...
	},
	{
		...
		notesEn:
			'100 typed instances per cycle. faker (manual) is raw-faster but requires 10-20 lines of per-model factory code, no type constraints, needs manual maintenance on schema changes. QM is built-in: zero setup, fully typed, respects field constraints automatically. Bar for faker is clipped — it runs at 80k ops/s.',
		notesEs:
			'100 instancias tipadas por ciclo. faker (manual) es más rápido en bruto pero requiere 10-20 líneas de factory por modelo, sin restricciones de tipo, requiere mantenimiento manual al cambiar el schema. QM está integrado: cero setup, totalmente tipado, respeta restricciones automáticamente. La barra de faker está recortada — corre a 80k ops/s.',
