...
| 12  | `guide/collection.md`                  | `QCollection.from()`; `.filter()`; `.mapBy()`; `.groupBy()`; `.sortBy()`; namespace `$qm` en colecciones; `$qFromArray()` roundtrip                                                                               | `integration/guide/collection.test.ts`          | ❌ Pendiente |
...
| 19  | `guide/tracing.md`                 | `$qm.trace()` devuelve campos transformados con origen; campos sin transformación no aparecen en traza; herencia: traza incluye campos de padre y de hijo; desactivar tracing no rompe el modelo     | `integration/guide/tracing.test.ts`                 | ❌ Pendiente |
