...
model.$qCheckRules({ group: 'identidad' });
// { valid: false, errors: [{ field: 'nombre', ... }] }

// Solo el grupo 'seguridad':
model.$qCheckRules({ group: 'seguridad' });
// { valid: true, errors: [] }
```
...
model.$qCheckRules({ group: Groups.identidad });
```
...
```typescript
if (!usuario.$qHasIntegrity()) {
	const errors = usuario.$qCheckIntegrity();
...
if (!usuario.$qIsValid()) {
	...
	const integrityErrors = usuario.$qCheckIntegrity(); // nivel tipo
	const ruleErrors = usuario.$qCheckRules().errors; // lógica de negocio
}
```
...
```typescript
const informe = usuario.$qValidationReport();
...
const updated = user.$qCopy({ edad: -5 });
...
// Evaluar
const resultado = await usuario.$qCheckRulesAsync();
if (!resultado.valid) {
...
>
> await form.$qCheckRulesAsync().then((r) => r.valid);
...
const resultado = await usuario.$qCheckRulesAsync({ mode: 'serial' });
```
...
```typescript
if (await usuario.$qIsValidAsync()) {
...
```typescript
const reporte = await usuario.$qValidationReportAsync();
if (!reporte.valid) {
