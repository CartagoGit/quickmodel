...
// Serializar
const plain = user.$qSerialize(); // devuelve el mismo shape con claves originales (snake_case)
...
console.log(form.$qSerialize()); // { first_name: 'Carlos', last_name: 'López', ... }
```
...
const result = userForm.$qCheckRules();
