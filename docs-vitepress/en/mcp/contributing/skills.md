...
> [!WARNING]
> ⚠️ **Internal Use Only**: These skills are for **QuickModel contributors**. They depend on [Internal Tools](./tools) and enforce project rules (TDD gates, lint, typecheck, naming conventions). They are not useful for developers who install the package via npm.
