...
// Send a message
const outgoing = new ChatMessage({ id: crypto.randomUUID(), text: 'Hello!', sentAt: new Date() });
>>>>>>> Stashed changes
...
- [FormData & Streaming](./formdata) — binary field streaming
- [URL Search Params (fromURL)](./fromurl) — parsing query strings
- [Validation](./validation) — `checkRules()` and `isValid()`
