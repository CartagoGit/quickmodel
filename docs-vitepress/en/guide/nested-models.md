...
`serialize()` recursively serializes all nested models (returns a plain object). Use `toJSON()` if you need a JSON string.
...
const plain = root.$qm.serialize(); // Works without infinite recursion
```
