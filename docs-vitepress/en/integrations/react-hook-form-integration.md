...
function EditProfileForm({ initialData }: { initialData: IUserSignup }) {
	...
	const handleSave = async () => {
		if (!dto.isDirty()) return; // nothing changed
		await saveProfile(dto.serialize());
		dto.reset(); // clear dirty state
		setDto(new UserSignupDto(dto.serialize() as IUserSignup));
	};
